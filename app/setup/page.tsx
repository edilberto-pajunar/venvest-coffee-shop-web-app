...
import { Printer as PrinterType } from "../printer/interface/Printer";
...
export default function SetupPage() {
  ...
  const handleSave = async () => {
    if (!printerId) {
      setSaveMessage("⚠️ Printer ID is required to save.");
    ...
    try {
      const existingPrinter = printers.find(p => p.id === printerId.toUpperCase());
      
      const autoGeneratedUrl = `${window.location.origin}/printer/${printerId.toUpperCase()}`;
      
      const newPrinter: PrinterType = {
        id: printerId.toUpperCase(),
        label: location || `Printer ${printerId.toUpperCase()}`,
        isOnline: false,
        url: autoGeneratedUrl
      };

      if (existingPrinter) {
        await PrinterService.updatePrinter(newPrinter);
        setSaveMessage("✅ Printer updated successfully!");
      } else {
        ...
        <div className="p-6 rounded-lg shadow-sm border" style={{ backgroundColor: COFFEE_PALETTE.cardBg, borderColor: COFFEE_PALETTE.border }}>
          ...
          <div className="space-y-4">
            <div>
              ...
              />
              <p className="text-xs mt-1" style={{ color: COFFEE_PALETTE.textSecondary }}>3-4 character unique identifier</p>
            </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <ol className="text-sm space-y-1" style={{ color: COFFEE_PALETTE.textSecondary }}>
              <li>1. Enter unique printer ID (3-4 characters)</li>
              <li>2. Optional: Set location name</li>
              <li>3. Click &ldquo;Save Printer&rdquo; to register in system</li>
              <li>4. Go to Printers page to view QR code and connection URL</li>
              <li>5. Copy the connection URL for staff access</li>
